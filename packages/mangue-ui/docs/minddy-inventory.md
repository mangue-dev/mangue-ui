# minddy → mangue-ui inventory

What minddy (`/Users/clementguerin/Projets/minddy-ticketing/minddy`) kept local
that actually belongs to the library, and what became of each piece in 0.3.0.

At the time of the audit minddy already consumed mangue-ui heavily (221
...
the minddy side: for every ported line, the `mangue-ui` import path to replace
the local one with.
...
- **No app vocabulary.** minddy's 67-tool table, its context-kind colors, its
  model allowlist are minddy's. The library ships the *mechanics* and takes the
...
| `components/assistant/tool-call-display.tsx` | *(new)* `src/ai/tool-call-list.tsx` | **adapt** — the display mechanics ported, the 67-entry `TOOL_META` table stays in minddy and arrives as `registry`; `ask_user` hooks back in through `renderExtra` | `ToolCallList`, `ToolCallItem`, `ToolMeta`, `toolRunningLabel` |
| `components/assistant/context-pill.tsx` | *(new)* `src/ai/context-pill.tsx` | **adapt** — the `STYLES` table (issue / objective / cycle…) stays in minddy and arrives as `visual` | `ContextPill` |
| `components/assistant/assistant-context-bar.tsx` (the row, l. 338-382) | *(new)* `src/ai/context-pill-row.tsx` | **extract** — `AddContextButton` (projects / members / issues data) stays in minddy and drops into the `action` slot | `ContextPillRow` |
...
| `components/assistant/chat-input.tsx` (the composer shell) | *(new)* `src/ai/agent-input.tsx` | **extract** the shell only — the `@` mentions, the `/` menu and the attachments are data-coupled and stay in minddy | `AgentInput` |
...
`minddy/tsconfig.json` carries a `"mangue-ui/*": ["./node_modules/mangue-ui/src/*"]`
...
- **`components/wizard-stepper.tsx` is a near-exact duplicate** of
  ...
  library's version is in fact slightly better (it has a focus ring the minddy
...
- **`components/category-pill.tsx` is NOT the same component** as
  ...
  *filter toggle* (`active` / `onClick` / `aria-pressed`); minddy's is a
...
- **`components/assistant/work-accordion.tsx:81-85` documented a library bug**:
  ...
  for any consumer. minddy worked around it by re-declaring the class locally.
  Fixed at the source (`cn()` with a plain literal); the workaround can go.

## Deliberately left in minddy

| minddy | why |
| --- | --- |
| `lib/command-palette/` (~30 files) + `command-palette.css` | Coupled to minddy's entities and its own theme file. A generic palette is a separate piece of work. |
...
| `lib/ask-user.ts` + `AskUserSummaryRow` | Coupled to minddy's ask-user protocol. Hooks back in through `ToolCallList`'s `renderExtra`. |
