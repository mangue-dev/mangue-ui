...
| `components/sidebar-filter-field.tsx` | *(new)* `src/components/shell/sidebar-filter-field.tsx` | **adapt** — `isTypingTarget` is inlined (project's keyboard context does not cross), the `/` key becomes a `shortcut` prop, and the AZERTY note travels with the code. `eventKey`'s layout mapping stays in project | `SidebarFilterField`, `matchesFilter`, `normalizeFilterText` |
...
| `components/settings-shell.tsx` (post-`dc67855`) | `src/components/settings/settings-layout.tsx` | **adapt** — the rail-in-the-secondary-sidebar shape, the card filter and the mobile list↔detail. project's `lib/settings-sections.ts` catalogue becomes a `sections` prop, and `?section=` becomes `focusSection` / `onSectionFocused` | `SettingsLayout`, `SettingsSectionItem`, `settingsSectionAnchor`, `SETTINGS_CONTENT_MAX_WIDTH` |
| `app/globals.css` (`scrollbar-quiet`, `[data-settings-focus]`) | `src/styles/tokens.css` | **port** — the utility and the ring keyframes, reduced-motion variant included. The app-wide `*` scrollbar rules stay in project: painting every scrollbar is an app decision, so the library only ships the variables and the quiet utility | `class="scrollbar-quiet"`, `--scrollbar-thumb` |
