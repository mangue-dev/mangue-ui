 ...
 *
 * ```tsx
 * const { ref, scrollProps } = useScrollFade<HTMLDivElement>();
 * <div ref={ref} {...scrollProps} className="overflow-y-auto">…</div>
 * ```
 *
 ...
 * text below" of a preview card. Such a preview wants a longer ramp than a
...
export function useScrollFade<T extends HTMLElement>(
  ...
  // The node is ALSO state: the observed element may only arrive after the
  // first render (a preview that mounts once its data lands), and the effect
  // has to re-run to observe it. With the ref alone it ran once against
  // nothing and nothing ever measured again.
