...
const tabsListVariants = cva(
  "group/tabs-list relative isolate inline-flex w-fit items-center justify-center gap-1 text-muted-foreground group-data-vertical/tabs:h-fit group-data-vertical/tabs:flex-col group-data-vertical/tabs:items-stretch",
  {
    variants: {
      variant: {
        default:
          "rounded-full bg-control p-1 group-data-horizontal/tabs:h-11 group-data-vertical/tabs:rounded-xl",
...
const tabsIndicatorVariants = cva(
  "pointer-events-none absolute top-0 left-0 -z-10 will-change-transform motion-reduce:transition-none",
  {
    variants: {
      variant: {
        default:
          "rounded-full bg-background shadow-sm group-data-vertical/tabs:rounded-lg",
...
function TabsTrigger({
  ...
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        "relative inline-flex flex-1 shrink-0 items-center justify-center gap-1.5 rounded-full bg-clip-padding px-4 text-sm font-medium leading-none whitespace-nowrap text-foreground/60 transition-all outline-none select-none group-data-horizontal/tabs:h-full group-data-vertical/tabs:w-full group-data-vertical/tabs:justify-start group-data-vertical/tabs:rounded-lg group-data-vertical/tabs:py-2 hover:text-foreground focus-visible:outline-none focus-visible:ring-0 disabled:pointer-events-none disabled:opacity-50 dark:text-muted-foreground dark:hover:text-foreground [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
